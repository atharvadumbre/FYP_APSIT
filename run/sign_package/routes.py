...
def register():
	...
	if forms.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		#flash(f'Account succesfully created, you can login now')
...
def model_test():
    ...
    if request.method == 'POST':
        ...
        if request.form['model'] == 'Get Question':
            global test
            test = np.random.randint(0,36)
            question =  "Please perform the character: " +  letters[test]
            return render_template('Model_Test.html', title='detection', question = question)
        elif request.form['model'] == 'Upload': 
            ...
            if file and allowed_file(file.filename):
                ...
                if (test <= 9):
                    ...
                    if(prediction[0][0] == letters[test]):
                        resul = "Congratulations, your sign is correct (predicted letter: " + prediction[0][0] +")"
                    else:
                        resul = "Unfortunately, your sign is incorrect (predicted letter: " + prediction[0][0] +")"
                    ...
                    if(prediction[0][0] == letters[test]):
                        ...
                        resul = "Unfortunately, your sign is incorrect (predicted letter: " + prediction[0][0] +")"
                    return render_template('Model_Test.html', title='detection', pred = resul)
    return render_template('Model_Test.html', title='detection')